...
class EquipmentTag(models.Model):
    ...
    _name = "durpro_fso.equipment.tag"
...
class EquipmentType(models.Model):
    _name = 'durpro_fso.equipment.type'
...
class EquipmentLocation(models.Model):
    _name = 'durpro_fso.equipment.location'
    ...
    def _compute_equipment_count(self):
        for rec in self:
            all_equipmemt_ids = self.env['durpro_fso.equipment'].search([('equipment_location_id', '=', rec.id)])
            rec.equipment_count = len(all_equipmemt_ids)


class Equipment(models.Model):
    _name = 'durpro_fso.equipment'
    ...
    tag_ids = fields.Many2many('durpro_fso.equipment.tag',
                               string='Application',
                               tracking=True,
    ...
    equipment_location_id = fields.Many2one('durpro_fso.equipment.location',
                                            string="Location",
                                            tracking=True,
