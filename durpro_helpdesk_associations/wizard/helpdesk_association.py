...
class HelpdeskTicketAssociation(models.TransientModel):
    ...
    work_order_id = fields.Many2one('durpro_fso.work_order', string="Work Order")
    ...
    def action_associate(self):
        ...
        elif self.associated_record == 'work_order':
            if not self.work_order_id:
                raise ValidationError(_("You must select an associated record."))
            self.work_order_id.helpdesk_ticket_id = self.ticket_id
